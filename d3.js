...
const GUN_PEERS = [
  ...
  // Funzione per verificare la versione
  ...
  // Funzione per attendere che ShogunCore sia disponibile
  function waitForShogunCore() {
    return new Promise((resolve, reject) => {
      let attempts = 0;
      const maxAttempts = 20; // 10 secondi di attesa max
      
      function checkShogunCore() {
        attempts++;
        
        if (typeof window.ShogunCore !== 'undefined') {
          // Verifica se abbiamo l'export nominale o default
  ...
  async function initializeShogunD3() {
    console.log("Attempting to patch d3.js");
    try {
      console.log(import.meta.env.VITE_GUN_TOKEN)
      ...
      if (!window.gun) {
        ...
        try {
          ...
          window.gun = window.gun.get("shogun").get("d3")
      ...
      // Inizializza SEA se non è disponibile
      if (!window.SEA && window.Gun && window.Gun.SEA) {
        window.SEA = window.Gun.SEA;
        console.log("✓ SEA inizializzato");
      }

      // Attendi che ShogunCore sia disponibile
      const ShogunCoreClass = await waitForShogunCore();
      
      if (!ShogunCoreClass || typeof ShogunCoreClass !== 'function') {
        throw new Error("ShogunCore non è una classe valida dopo l'inizializzazione");
      }

      // Ora possiamo usare ShogunCore con la configurazione corretta
      const shogunInstance = new ShogunCoreClass({
        peers: GUN_PEERS,
        web3: {
          enabled: true
        },
        ...
        // Aggiungiamo la configurazione per Gun
        gunInstance: window.gun,
        scope: 'shogun-d3'
      });

      if (!shogunInstance || !shogunInstance.gun) {
        throw new Error("Inizializzazione ShogunCore fallita - istanza non valida");
      }

      console.log("✓ ShogunCore inizializzato con successo");
      
      // Recupera l'istanza di Gun da ShogunCore e configura il monitoraggio
      ...
      window.d3 = {
        ...
        // NUOVA FUNZIONE: Connessione wallet tramite ShogunCore (AGGIORNATO)
        connectWithMetaMask: async function() {
          try {
            ...
            // Ottieni il plugin Web3 per l'autenticazione MetaMask
            const web3Plugin = this.shogun.getPlugin("web3");
            if (!web3Plugin) {
              throw new Error("Plugin Web3 non disponibile. Assicurati che sia abilitato nella configurazione.");
            }
            ...
            // Ottiene il keypair Gun gestito da ShogunCore (AGGIORNATO)
        ...
        // FUNZIONI DI BACKUP/RESTORE - NOTA: Richiedono plugin specifici (es. HDWallet)
        backupKeypair: async function(password) {
          ...
          // Verifica se è disponibile un plugin con funzionalità di backup
          const hdWalletPlugin = this.shogun.getPlugin("hdwallet");
          if (hdWalletPlugin && hdWalletPlugin.exportGunPair) {
            return await hdWalletPlugin.exportGunPair(password);
          }
          
          // Fallback per backup manuale del keypair
          const gunUser = this.shogun.user;
          if (gunUser?._.sea) {
            const keypair = {
              pub: gunUser._.sea.pub,
              priv: gunUser._.sea.priv,
              epub: gunUser._.sea.epub,
              epriv: gunUser._.sea.epriv
            };
            
            if (password) {
              // Crittografa se è fornita una password
              const encrypted = await window.SEA.encrypt(JSON.stringify(keypair), password);
          ...
          throw new Error("Nessun keypair disponibile per il backup");
        },
        
        // NUOVA FUNZIONE: ripristina keypair (AGGIORNATA)
        restoreKeypair: async function(backupData, password) {
          try {
            // Verifica se è disponibile un plugin con funzionalità di restore
            const hdWalletPlugin = this.shogun.getPlugin("hdwallet");
            if (hdWalletPlugin && hdWalletPlugin.importGunPair) {
              const result = await hdWalletPlugin.importGunPair(backupData, password);
              ...
              if (result) {
                ...
                if (window.currentUserAddress && window.gunKeyPair) {
            ...
            // Fallback per restore manuale
            console.warn("Plugin HDWallet non disponibile, tentativo di restore manuale");
            const backupObj = JSON.parse(backupData);
            
            let keypair;
            if (backupObj.type === "encrypted-gun-pair" && password) {
              const decrypted = await window.SEA.decrypt(backupObj.data, password);
              if (!decrypted) {
                throw new Error("Password non valida o dati corrotti");
              }
              keypair = JSON.parse(decrypted);
            } else if (backupObj.type === "plain-gun-pair") {
              keypair = backupObj.data;
            } else {
              throw new Error("Formato backup non riconosciuto");
            }
            
            // Autentica con il keypair ripristinato
            // Nota: questo è un metodo semplificato e potrebbe non funzionare in tutti i casi
            if (keypair.pub && keypair.priv) {
              const user = this.shogun.gun.user();
              await new Promise((resolve, reject) => {
                user.auth(keypair, (ack) => {
                  if (ack.err) {
                    reject(new Error(`Errore durante l'autenticazione: ${ack.err}`));
                  } else {
            ...
            throw new Error("Keypair non valido nei dati di backup");
          } catch (error) {
            console.error("Errore durante il restore del keypair:", error);
            throw error;
          }
        },
        ...
        createSharedSecret: async function (recipientAddress, gunKeypair) {
          ...
          if (
            ...
            try {
              ...
              const secret = await window.SEA.encrypt(epub, gunKeypair);
        ...
        sendmessage: async function (payload, to, gunKeypair, customMessageKey) {
          ...
          try {
            ...
            const messageKey = customMessageKey || `msg_${Date.now()}_${Math.random().toString(36).substring(2, 9)}`;
      ...
      // Imposta il livello di log predefinito dopo che d3 è stato creato
      if (window.d3 && window.d3.debug) {
        window.d3.debug.setLogLevel = window.d3.debug.setLogLevel || function(level) {
      ...
      // Setup del wrapper per receiveMessage dopo l'inizializzazione
      if (window.d3 && window.d3.receiveMessage) {
    ...
    // Inizializza Gun immediatamente se non è stato già fatto
  if (!window.gun) {
    try {
      window.gun = Gun({
        localStorage: false,
  ...
  function createLogger() {
    ...
    function log(level, ...args) {
      // Prendiamo il livello di log attualmente configurato, con fallback sicuro
      let configuredLevel = 'info';
      try {
        if (window.d3 && window.d3.debug && window.d3.debug.logLevel) {
          configuredLevel = window.d3.debug.logLevel;
        }
      } catch (e) {
    ...
    return {
      ...
      setLogLevel: (level) => {
        try {
          if (!window.d3 || !window.d3.debug) {
            console.warn("[SHOGUN-D3:WARN] d3.debug not available yet");
            return;
          }
          ...
          console.error("[SHOGUN-D3:ERROR] Error setting log level:", e);
        }
      },
      getLogLevel: () => {
        try {
          return window.d3?.debug?.logLevel || 'info';
        } catch (e) {
          return 'info';
        }
      },
  ...
  // Avvia l'inizializzazione quando il file viene caricato
  initializeShogunD3();
